...
export const config: Config = {
	...
	spr: {
		enabled: process.env.PN_BOSS_CONFIG_STREETPASS_RELAY_ENABLED?.trim() === 'true'
	},
};
...
if (!config.spr.enabled) {
	warnings.push('Streetpass Relay has been enabled by default. To disable SPR, set the PN_BOSS_CONFIG_STREETPASS_RELAY_ENABLED environment variable to false');
	disabledFeatures.spr = false;
}

if (config.spr.enabled == false) {
	warnings.push('Streetpass Relay has been disabled. To enable SPR, set the PN_BOSS_CONFIG_STREETPASS_RELAY_ENABLED environment variable to true');
